...
def tacos(total=0, order=""):

    try:
        item = input(f"Item: ").title()
    except EOFError:

        #When pressing Control+D ends program. Prints new line to input, shows user's order and total on different lines
        print(f"", end="\n")
        print(f"Your order: {order[0:len(order)-2]}", end="\n")
        print(f"Total: ${total:.2f}", end="\n")
        return

    #finds the item on the menu and adds its price to the menu, if it isn't found reprompts user
    if item in menu:
        total += menu[item]
